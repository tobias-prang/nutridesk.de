...
app.get('/bootstrap', auth, asyncRoute(async (req, res) => {
  ...
  const [user, settings, weights, water, foodLog, dishes, plan, shopping,
    ...
    q("SELECT id,transaction_id,folder_id,name,size,category,created_at FROM cloud_files WHERE user_id=? AND folder_id IS NOT NULL AND scan_status IN ('clean','legacy_unverified') ORDER BY created_at DESC"),
