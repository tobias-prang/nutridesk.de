...
(async()=>{const conn=await pool.getConnection();try{await conn.execute('ALTER TABLE assistant_notes ADD COLUMN IF NOT EXISTS pinned TINYINT(1) NOT NULL DEFAULT 0 AFTER search_aliases_json');await conn.beginTransaction();const [[user]]=await conn.execute('SELECT id FROM users WHERE LOWER(username)=LOWER(?)',['tobiasprang']);if(!user)throw new Error('Konto tobiasprang nicht gefunden');
const sectionTitleByKey={'important-glossary':'TV-Wissen','important-network-levels':'Kabelnetz & Breitbandwissen','important-titan-router':'Systeme','important-tv-flow':'Störung & Analyse','important-mmd-hardware':'Kabelnetz & Breitbandwissen','important-router-ports':'Kabelnetz & Breitbandwissen','important-sales-after-care':'Vertrag & Angebot'};
const [sections]=await conn.execute('SELECT id,title FROM assistant_note_sections WHERE user_id=?',[user.id]);const sectionByTitle=new Map(sections.map(s=>[s.title,Number(s.id)]));
const ids=new Map();for(let i=0;i<notes.length;i++){const n=notes[i],sectionId=sectionByTitle.get(sectionTitleByKey[n.key]||'Grundlagen');if(!sectionId)throw new Error('Zielkategorie fehlt: '+sectionTitleByKey[n.key]);let [[row]]=await conn.execute('SELECT id FROM assistant_notes WHERE user_id=? AND external_id=?',[user.id,n.key]);if(!row&&n.key==='important-glossary')[[row]]=await conn.execute('SELECT id FROM assistant_notes WHERE user_id=? AND title IN (?,?)',[user.id,'Kabel-TV-Glossar – die wichtigsten Begriffe',n.title]);if(row)await conn.execute('UPDATE assistant_notes SET section_id=?,external_id=?,title=?,content=?,tags_json=?,search_aliases_json=?,pinned=1,sort_order=? WHERE id=?',[sectionId,n.key,n.title,n.content,JSON.stringify(n.tags),JSON.stringify(n.aliases),i,row.id]);else{const [r]=await conn.execute('INSERT INTO assistant_notes (user_id,section_id,external_id,title,content,tags_json,search_aliases_json,pinned,sort_order) VALUES (?,?,?,?,?,?,?,?,?)',[user.id,sectionId,n.key,n.title,n.content,JSON.stringify(n.tags),JSON.stringify(n.aliases),1,i]);row={id:r.insertId};}ids.set(n.key,row.id);}
...
const imageDir=path.join(process.env.NOTE_IMAGE_ROOT||'/home/nutridesk.de/app/assets/notes',String(user.id));await fsp.mkdir(imageDir,{recursive:true});
...
await conn.commit();console.log(JSON.stringify({ok:true,pinned:true,notes:[...ids.values()]}));}catch(e){await conn.rollback();throw e;}finally{conn.release();await pool.end();}})().catch(e=>{console.error(e.stack||e.message);process.exit(1);});
